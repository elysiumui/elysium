...
Native UI: select a mesh, open Modifiers, choose Add WeightedNormals, edit Mode/Weight/Threshold/Keep sharp and Update. A visible change requires smooth shading. Each mutation participates in normal Undo/Redo. Public authoring uses `mesh.modifier_add` and `mesh.modifier_update` with kind `WeightedNormals`; `mesh.modifiers_get` includes evaluated corner vectors.
