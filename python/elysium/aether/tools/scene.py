...
@register_tool(
    ...
    input_schema={"type":"object","additionalProperties":False,"properties":{
        ...
        "distance":{"type":"number"},"offset":_VECTOR,"position":_VECTOR,"rotation":_VECTOR,"scale":_VECTOR,"radius":{"type":"number","minimum":0}},"required":["id","operation"]},
)
def components_edit(session,id,operation,distance=1.0,offset=(0.,0.,0.),position=(0.,0.,0.),radius=0.0,rotation=(0.,0.,0.),scale=(1.,1.,1.)):
    from ...render import topology
    return topology.edit_selected(session.lookup(id),operation,distance=distance,offset=offset,position=position,radius=radius,rotation=rotation,scale=scale)
