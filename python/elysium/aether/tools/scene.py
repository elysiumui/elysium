...
@register_tool(
    ...
    input_schema={"type":"object","additionalProperties":False,"properties":{
        ...
        "threshold":{"type":"number","minimum":0},"centroid":{"type":"boolean"},"distance":{"type":"number"},"offset":_VECTOR,"position":_VECTOR,"rotation":_VECTOR,"scale":_VECTOR,"radius":{"type":"number","minimum":0},"cuts":{"type":"integer","minimum":1,"maximum":64},"segments":{"type":"integer","minimum":1,"maximum":64},"factor":{"type":"number","exclusiveMinimum":-1,"exclusiveMaximum":1},"plane_point":_VECTOR,"plane_normal":_VECTOR,"keep":{"enum":["both","negative","positive"]},"fill":{"type":"boolean"}},"required":["id","operation"]},
)
def components_edit(session,id,operation,distance=1.0,offset=(0.,0.,0.),position=(0.,0.,0.),radius=0.0,rotation=(0.,0.,0.),scale=(1.,1.,1.),cuts=1,factor=0.0,segments=1,plane_point=(0.,0.,0.),plane_normal=(1.,0.,0.),keep="both",fill=False,threshold=0.0001,centroid=True):
    from ...render import topology
    return topology.edit_selected(session.lookup(id),operation,distance=distance,offset=offset,position=position,radius=radius,rotation=rotation,scale=scale,cuts=cuts,factor=factor,segments=segments,plane_point=plane_point,plane_normal=plane_normal,keep=keep,fill=fill,threshold=threshold,centroid=centroid)
