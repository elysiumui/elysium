...
@register_tool(name="material.slot_image_set", description="Import a single PNG/JPEG (at most 2048x2048, 16 MiB) as this material slot's owned image for base_color (default), roughness, metallic or normal. Stored in the editable project and native export; Base color uses sRGB RGB; roughness/metallic use linear red-channel data; normal uses linear RGB tangent-space directions at strength 1. Closest sampling, repeat UVs, opaque surface; color/scalar images multiply their matching surface values; normal images alter shading without editing geometry. Empty path clears the image override. Other surfaces and source attributes remain unchanged.", input_schema={"type":"object","additionalProperties":False,"properties":{"id":{"type":"string"},"slot_id":{"type":"string"},"path":{"type":"string"},"channel":{"enum":["base_color","roughness","metallic","normal"]}},"required":["id","slot_id","path"]})
