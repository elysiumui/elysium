...
4. **Single colour, recolourable** — draw in one colour (`currentColor`); don't bake palette colours (the theme tints to foreground, accent only marks the active item). Max one extra tone at ~60% opacity.
...
| ![tool_gizmo](current/tool_gizmo.svg) | `tool_gizmo` | Rotate (E) — monoline circular-arrows glyph (replaces the old multi-colour 3-ellipse manipulator for a cohesive Studio set). |
...
| ![tool_pivot_edit](current/tool_pivot_edit.svg) | `tool_pivot_edit` | Pivot Edit — crosshair + centre pin, monoline. |
...
| ![tb_snap_grid](current/tb_snap_grid.svg) | `tb_snap_grid` | 3×3 grid of dots with the centre dot highlighted accent. |
| ![tb_soft_select](current/tb_soft_select.svg) | `tb_soft_select` | Concentric circles fading centre-to-edge (gradient falloff). |
