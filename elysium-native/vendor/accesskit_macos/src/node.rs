...
declare_class!(
    ...
    unsafe impl PlatformNode {
        ...
        fn set_value(&self, value: &NSObject) {
            if !value.is_kind_of::<NSString>() {
                return;
            }
            // The runtime class check establishes an NSString-compatible object.
            // Borrow it immutably and copy the contents before dispatching.
            let replacement = unsafe { &*(value as *const NSObject as *const NSString) }
                .to_string();
            self.resolve_with_context(|node, context| {
                if !node.is_read_only_or_disabled()
                    && matches!(node.role(), Role::TextInput | Role::SearchInput | Role::MultilineTextInput)
                {
        ...
        fn is_selector_allowed(&self, selector: Sel) -> bool {
            self.resolve(|node| {
                ...
                if selector == sel!(setAccessibilityValue:) {
                    // Preserve editable rich-text behavior and expose replacement for
                    // plain text controls that do not publish text-range children.
                    return !node.is_read_only_or_disabled()
