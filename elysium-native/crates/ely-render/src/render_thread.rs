...
fn render_loop(
    ...
    loop {
        ...
        let damage = if !dirty_enabled {
            ...
            if anim_active { Damage::Full } else { crate::damage::diff_damage(&prev, &cached) }
        } else if anim_active {
        ...
        let phys: Option<(u32, u32, u32, u32, f32, f32, f32, f32)> = match damage {
            ...
            Damage::Rect(lx, ly, lw, lh) => {
                ...
                if pw == 0 || ph == 0
                    || (pw as u64 * ph as u64) * 5 >= (w as u64 * h as u64) * 3 {
                    None
                } else {
                    Some((x0 as u32, y0 as u32, pw, ph, lx - pad, ly - pad, lw + 2.0 * pad, lh + 2.0 * pad))
                }
            }
        };
        ...
        if let Some((px, py, pw, ph, clx, cly, clw, clh)) = phys {
            ...
            if !skia.snapshot_region_bgra(px, py, pw, ph, &mut region_buf) {
                let (sw, sh) = skia.size();
                if (sw, sh) != (w, h) { skia = SkiaLayer::new(w, h); }
                prev = DisplayList::default();
                continue;
            }
