...
impl TransformValue {
    pub const IDENT: Self = Self {
        tx: 0.0, ty: 0.0, sx: 1.0, sy: 1.0, rotation: 0.0, alpha: 1.0,
    };
}
...
impl AnimRegistry {
    ...
    pub fn set_target(&self, slot: u32, to: TransformValue,
                      duration_secs: f32, easing: Easing) {
        let mut g = self.slots.lock();
        let from = g.get(&slot).map(|s| s.current).unwrap_or(TransformValue::IDENT);
        g.insert(slot, SlotState {
            from, to, start: Instant::now(),
    ...
    pub fn snap(&self, slot: u32, v: TransformValue) {
        let mut g = self.slots.lock();
        g.insert(slot, SlotState {
            from: v, to: v, start: Instant::now(),
            duration: 1e-3, easing: Easing::Linear, current: v,
        });
    }
    ...
    pub fn clear(&self, slot: u32) { self.slots.lock().remove(&slot); }
}
...
mod tests {
    ...
    fn set_and_snap() {
        ...
        r.snap(7, TransformValue { tx: 100.0, ..TransformValue::IDENT });
        let v = r.current(7).unwrap();
        assert_eq!(v.tx, 100.0);
        r.set_target(7, TransformValue { tx: 200.0, ..TransformValue::IDENT },
                     0.05, Easing::Linear);
