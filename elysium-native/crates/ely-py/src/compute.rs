...
pub fn render_pbr_compute<'py>(
    ...
    if uniforms.len() != 32 {
        return Err(PyRuntimeError::new_err(format!(
            "uniforms must be 32 floats, got {}", uniforms.len())));
    }
    let u = uniforms;
    let uni = Uniforms {
    ...
    let verts4: Vec<[f32; 4]> = verts.chunks_exact(4)
        .map(|c| [c[0], c[1], c[2], c[3]]).collect();
    let faces4: Vec<[u32; 4]> = faces.chunks_exact(4)
        .map(|c| [c[0], c[1], c[2], c[3]]).collect();
    let normals4: Vec<[f32; 4]> = normals.chunks_exact(4)
        .map(|c| [c[0], c[1], c[2], c[3]]).collect();
    let bmin4: Vec<[f32; 4]> = bvh_min.chunks_exact(4)
        .map(|c| [c[0], c[1], c[2], c[3]]).collect();
    let bmax4: Vec<[f32; 4]> = bvh_max.chunks_exact(4)
        .map(|c| [c[0], c[1], c[2], c[3]]).collect();
    let bmeta4: Vec<[i32; 4]> = bvh_meta.chunks_exact(4)
        .map(|c| [c[0], c[1], c[2], c[3]]).collect();
    let mats: Vec<GpuMaterial> = materials.chunks_exact(16)
        .map(|c| GpuMaterial {
    ...
    let eng = guard.as_ref().ok_or_else(||
        PyRuntimeError::new_err("wgpu compute device unavailable"))?;
    let pixels = py.allow_threads(|| {
        eng.render(w, h, uni,
                   &verts4, &faces4, &normals4,
                   &bmin4, &bmax4, &bmeta4, &tri_order, &mats)
    });
