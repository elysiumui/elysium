        ...
        for d in nodes {
            ...
            by_id.insert(
                ...
                ely_platform::a11y::A11yNode {
                    ...
                    description: desc,
                    shortcut: sc,
                    value: d.get_item("value")?.and_then(|v| v.extract::<String>().ok()),
                    disabled: d.get_item("disabled")?.and_then(|v| v.extract::<bool>().ok()).unwrap_or(false),
                    selected: d.get_item("selected")?.and_then(|v| v.extract::<bool>().ok()),
