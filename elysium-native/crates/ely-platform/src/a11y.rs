...
pub trait AccessibilityNode {
    ...
    fn keyboard_shortcut(&self) -> Option<&str> { None }
    /// Bounding box in window-local logical pixels: (x, y, w, h).
    fn bounds(&self) -> (f32, f32, f32, f32) { (0.0, 0.0, 0.0, 0.0) }
    /// Children, in tab order.
    fn children(&self) -> Vec<Box<dyn AccessibilityNode>> { Vec::new() }
}
...
impl A11yState {
    ...
    pub fn publish(&self, tree: A11yTree) {
        *self.tree.lock() = tree;
        self.tree_dirty.store(true, std::sync::atomic::Ordering::Release);
    }
    ...
    pub fn hit(&self, x: f32, y: f32) -> Option<A11yNode> {
        ...
        tree.root.as_ref().and_then(|n| hit_recurse(n, x, y).cloned())
    }
}

fn hit_recurse<'a>(node: &'a A11yNode, x: f32, y: f32) -> Option<&'a A11yNode> {
...
mod tests {
    ...
    fn n(id: u64, role: &str, b: (f32, f32, f32, f32), kids: Vec<A11yNode>) -> A11yNode {
        A11yNode { id, role: role.into(), label: None, description: None,
                   shortcut: None, bounds: b, children: kids }
    }

    #[test]
    fn hit_test_returns_deepest() {
        let state = A11yState::new();
        let tree = A11yTree { root: Some(n(1, "window", (0.0, 0.0, 800.0, 600.0), vec![
            n(2, "group", (10.0, 10.0, 200.0, 200.0), vec![
                n(3, "button", (20.0, 20.0, 60.0, 30.0), vec![]),
            ]),
        ]))};
        state.publish(tree);
